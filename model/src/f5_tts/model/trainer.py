...
class Trainer:
    ...
    def load_checkpoint(self):
        ...
        if self.is_main:
            load_res = self.ema_model.load_state_dict(checkpoint["ema_model_state_dict"], strict=False)
            if load_res.missing_keys:
                print(f"EMA Missing keys: {load_res.missing_keys}")
            if load_res.unexpected_keys:
                print(f"EMA Unexpected keys: {load_res.unexpected_keys}")
        ...
        if "update" in checkpoint or "step" in checkpoint:
            ...
            load_res = self.accelerator.unwrap_model(self.model).load_state_dict(checkpoint["model_state_dict"], strict=False)
            if load_res.missing_keys:
                print(f"Missing keys (expected for new modules): {load_res.missing_keys}")
            if load_res.unexpected_keys:
                print(f"Unexpected keys (expected for structural changes): {load_res.unexpected_keys}")
            # Skip optimizer loading if model architecture changed
            if len(load_res.missing_keys) > 0 or len(load_res.unexpected_keys) > 0:
                print("Model architecture changed (missing/unexpected keys detected). Skipping optimizer state loading.")
            else:
                try:
                    self.optimizer.load_state_dict(checkpoint["optimizer_state_dict"])
                except Exception as e:
                    print(f"Failed to load optimizer state: {e}\nSkipping optimizer loading, this is expected if you changed the model architecture.")
            if self.scheduler:
